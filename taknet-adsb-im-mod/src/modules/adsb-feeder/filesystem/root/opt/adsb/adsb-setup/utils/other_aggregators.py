...
class TaknetPS(Aggregator):
    """TAKNET-PS custom aggregator integration."""
    ...
    def _get_aggregator_host(self) -> str:
        """
        Automatically determine the aggregator host based on Tailscale status.
        Returns: tailscale.leckliter.net if Tailscale is active, otherwise adsb.leckliter.net
        """
        try:
            ...
            if result.returncode == 0 and result.stdout.strip() == "active":
                ...
                if result.returncode == 0:
                    ...
                    # Check if we have a valid BackendState (connected)
                    if status.get("BackendState") in ["Running", "Starting"]:
                        print_err("TAKNET-PS: Tailscale is active, using tailscale.leckliter.net")
                        return "tailscale.leckliter.net"
        except Exception as e:
            print_err(f"TAKNET-PS: Error checking Tailscale status: {e}")
        
        # Default to non-Tailscale host
        print_err("TAKNET-PS: Tailscale not active, using adsb.leckliter.net")
        return "adsb.leckliter.net"

    def _activate(self, user_input: str, idx: int = 0) -> bool:
        """
        Activate TAKNET-PS with port configuration only.
        Host is automatically determined based on Tailscale status.
        user_input format: just the port number
        """
        self._idx = make_int(idx)
        port = user_input.strip()
        
        if not port:
            print_err("missing port for TAKNET-PS")
            flash("TAKNET-PS port is required")
            return False
        ...
        try:
            ...
            print_err(f"invalid port for TAKNET-PS: {port}")
            flash("TAKNET-PS port must be a number between 1 and 65535")
            return False
        
        # Automatically determine host based on Tailscale status
        host = self._get_aggregator_host()
        
        self._d.env_by_tags(self.tags + ["host"]).list_set(idx, host)
        self._d.env_by_tags(self.tags + ["port"]).list_set(idx, port)
        self._d.env_by_tags(self.tags + ["is_enabled"]).list_set(idx, True)
        
        # Flash a message to inform the user which host was selected
        flash(f"TAKNET-PS configured to send data to {host}:{port}")
        return True
